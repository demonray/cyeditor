...
class Clipboard {
  ...
  _init() {
    ...
    if (!scratchPad.isInitialized) {
      ...
      scratchPad.instance = {
        copy:  (eles, _id) => {
          let id = _id ? _id : this._getItemId();
          eles.unselect();
        ...
        paste:  (_id) => {
          let id = _id ? _id : this._getItemId(true);
  ...
  changeIds(jsons) {
    ...
    for (let j = 0; j < jsons.length; j++) {
      ...
      for (let k = 0; k < fields.length; k++) {
        let field = fields[k];
        if (json.data[field] && this._oldIdToNewId[json.data[field]])
          json.data[field] = this._oldIdToNewId[json.data[field]];
