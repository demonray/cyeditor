...
function generateInstance(cy: cytoscape.Core) {
  ...
  instance.undo = function () {
    if (!this.isUndoStackEmpty()) {
      ...
      let res = this.actions[action.name].undo(action.args)
  ...
  instance.redo = function () {
    if (!this.isRedoStackEmpty()) {
      ...
      let res = this.actions[action.name].doFn(action.args)
  ...
  instance.action = function (actionName: string, doFn: any, undo: any) {
...
function setDragUndo(cy: cytoscape.Core, undoable: any) {
  ...
  cy.on('free', 'node', function (e) {
    ...
    let target = e.target
    if (typeof undoable === 'function' ? undoable.call(target) : undoable) {
...
function defaultActions(cy: cytoscape.Core) {
  ...
  function getEles(eles: any) {
    return (typeof eles === 'string') ? cy.$(eles) : eles
  }

  function restoreEles(eles: any) {
    return getEles(eles).restore()
  }
  ...
  function batch(actionList: any, doOrUndo: any) {
    ...
    for (let i = 0; i < actionList.length; i++) {
      ...
      let actionResult
      if (doOrUndo === 'undo') {
        actionResult = actions[action.name].undo(action.param)
      } else {
        actionResult = actions[action.name].doFn(action.param)
      }
  ...
  return {
    ...
    'select': {
      doFn (eles: any) {
        return getEles(eles).select()
      },
      undo (eles: any) {
        return getEles(eles).unselect()
      }
    },
    'unselect': {
      doFn (eles: any) {
    ...
    'drag': {
      doFn (args: { move: any; positionDiff: any; nodes: any }) {
        if (args.move) { moveNodes(args.positionDiff, args.nodes) }
        return args
      },
      undo (args: { positionDiff: { x: number; y: number }; nodes: any }) {
        let diff = {
